...
def inference(w_tensor, p_tensor):
    ...
    with tf.variable_scope('layer1'):
        ...
        preactivations = tf.pow(tf.add_n([tf.matmul(w_tensor, w11),
                                          tf.matmul(p_tensor, w12)]) + b1, 3, name="preactivations")
...
def train():
    ...
    F = open("./Model_no_dep/train_result.txt", "w+")
    with tf.Session() as sess:
        ...
        for i in range(training_steps):
            ...
            if i % 1000 == 0:
                ...
                F.close()
                F = open("./Model_no_dep/train_result.txt", "a")
                f = open("./Model_no_dep/model.ckpt", "w+")
                f.close()
                saver.save(
                    # sess, os.path.join("./Model/", "model.ckpt"),
                    sess, "./Model_no_dep/model.ckpt",
                    global_step=i
                )
    F.close()
